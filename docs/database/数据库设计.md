...
| paid_amount | NUMERIC(18,2) | 已核销金额（含本金+滞纳金） |
| late_fee_amount | NUMERIC(18,2) | 累计滞纳金（汇总，明细见 `late_fee`） |
...
字段细节在 Flyway `V*` 脚本与 DSD 编码阶段展开；状态机见 SRS §4.24.4–§4.24.8。V1.2 新增 `late_fee`、`migration_batch`、`migration_import_log` 及 `bill_payment` 核销流水语义（对齐 SRS V2.4 §4.23.28/29、§4.23.6）。
