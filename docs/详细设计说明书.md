...
| L4 加密 | `FieldEncryptionService` AES-256-GCM；`id_no_hash` 盲索引 |
| 脱敏 | `DataMaskingInterceptor` 按角色掩码 phone/id_no |
| 导出 | `ExportService` 水印 + `export_audit`；L4 默认拒绝 |
| 存储 | PG 盘加密；OSS 私有 + SSE；Redis 不存 L3/L4 明文 |
| 日志 | pino `redact`：password、token、id_no、phone |
| 密钥 | 环境变量/KMS；禁止入库与日志 |
...
| V1.4 | 2026-08-26 | 数据安全强化：§7.2 对齐 SRS §5.8、数据安全设计说明书、ADR-0015 |
