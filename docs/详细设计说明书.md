...
SRS V2.4 全量功能（P0/P1/P2 + 十三条业务闭环 + 收费算法与期初迁移）；详见 [需求规格说明书.md](./需求规格说明书.md)。
...
| billing | 账单、收款、预收、核销 | BillService, PaymentService, PrepayService, AllocationService |
...
```typescript
function prorate(p: Period, terms: ContractTerms): Decimal {
  const days = diffDays(p.start, p.end) + 1;
  const denom = terms.prorationBase === 'calendar'
    ? daysInMonth(p.start)   // 当月自然日
    : 30;                     // 固定 30 天
  return round2(terms.monthlyRent * days / denom);
}
```
...
- PR：`mvn test` + `pnpm api:lint` + 前端 lint
