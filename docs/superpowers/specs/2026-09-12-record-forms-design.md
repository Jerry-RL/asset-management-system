...
> ⚠️ **2026-09-18 调整（已实施）**：下表「新增」一行**已下线**。产品口径：处置是**由处置动作产生的事实**，
> 不是可以随手追加的台账行 —— 面板上点几次就能造出多张处置卡，会让「资产处置记录」台账与真实发生过的
> 处置脱钩。因此**三种主体都不再提供「新增处置记录」按钮**，处置记录一律由业务动作写入：
> 资产走「资产处置」模块（完成时级联写台账），项目 / 分区走后端 `record-sheet` 的处置段。
> 「修改」「删除」「字段」三行不变；**资产侧的四个流程按钮保留** —— 否则处置状态机将失去唯一入口。
> 实现见 `RecordSheetSections.renderDisposals`；台账侧见 [资产处置记录设计](./2026-09-18-asset-disposal-record-design.md)。

| 交互 | 说明 |
|------|------|
| ~~新增~~ | ~~「新增处置记录」按钮在列表尾部**追加一张空卡**，字段就地编辑~~ → **已下线**，见上方调整说明 |
...
11. 三处处置记录**交互一致**：资产表单第 3 步、项目表单第 3 步、分区详情页都是「卡片列表」——~~新增 = 尾部追加空卡、~~字段就地编辑、删除按钮在卡片内，改动都攒到点保存才提交；资产卡片刻录的处置单在保存后落成 `disposal_order` 的 `draft`。⚠️ **2026-09-18 调整**：**新增入口已下线**（三种主体都不再有「新增处置记录」按钮），处置记录一律由业务动作写入，见 §6.6 的调整说明。
12. 资产侧流程按钮**先同步再推进**：在已有草稿的卡片上改了金额后直接点「提交审批」，提交成功后重新打开该资产，卡片与库里都是**新**金额（不允许出现「按新金额审批、库里留旧值」）。
13. 无 `operation.disposal:create` 的账号在资产表单第 3 步：处置卡片只读（既不显示新增入口，也不可改草稿），且保存时**不发** `PUT /assets/{id}/disposals`（保存接收信息不会报 403）。
