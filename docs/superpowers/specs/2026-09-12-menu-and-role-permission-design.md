...
| **`TableActions` 的 `perm` 只在新页面声明，其余 9 个调用点未逐一补声明** | 这些页面上的操作（如审批页的通过/驳回、收费确认页的确认）仍**只按后端 403 拦截，前端不做显隐** | 逐页补声明需要逐个判断每个操作对应哪个 `menuCode:action`（一张表里可能混着不同模块的动作，如「生成合同」实际要求的是 `contract.ledger:create` 而不是当前页的 code），猜错会把可用按钮藏掉。故本轮只落地机制 + 已确认的一处（系统菜单页），其余登记跟踪 |
