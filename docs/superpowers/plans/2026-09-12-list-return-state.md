...
4. 列表内翻页/改筛选**不产生历史条目**（`replace: true`，设计 §4.2）：从第 2 页翻回第 1 页后点浏览器**后退** → **退出列表**，回到进入「资产台账」之前的那一页（验收标准 4）。
...
8. 列表内翻页**不产生历史条目**（`replace: true`）：翻到第 2 页后点浏览器**后退** → **退出列表**，而不是回到第 1 页（验收标准 4）。
