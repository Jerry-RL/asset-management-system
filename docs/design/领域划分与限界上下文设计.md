...
| 关联 | [概要设计说明书.md](./概要设计说明书.md)、[详细设计说明书.md](../详细设计说明书.md) V1.8、[数据库设计.md](../database/数据库设计.md)、[ADR-0002](../adr/0002-monolith-modular-backend.md)、[ADR-0013](../adr/0013-in-process-domain-events.md)、[ADR-0019](../adr/0019-asset-unit-and-occupancy-model.md)、[ADR-0020](../adr/0020-bounded-contexts-and-aggregate-boundaries.md)、[ADR-0021](../adr/0021-asset-split-merge-rules.md) |
| 评审 | [领域划分与限界上下文评审报告.md](./领域划分与限界上下文评审报告.md)（V1.0 缺陷清单与处置） |
| 拆分合并专项 | [ADR-0021 资产与计租单元拆分合并规则](../adr/0021-asset-split-merge-rules.md)（本文 §7.4 的规则来源与 D-01~D-25 缺陷清单） |
...
- V1.0 为初版设想；**V1.1 依据[评审报告](./领域划分与限界上下文评审报告.md) P0/P1 缺陷修正**，主要变更：
  ...
  5. 补**十三条闭环映射矩阵**（§11.1）。
- **V1.2 依 [ADR-0021](../adr/0021-asset-split-merge-rules.md) 补齐拆分合并规则**，主要变更：
  1. 新增 **§7.4 拆分与合并**（三层语义、单元拆合规则 S1–S11 / M1–M7、资产层收窄规则 A1–A9）；
  2. 新增不变量 **INV-13~INV-18**（§7.3）；
  3. 修正术语表 `AssetUnit` 的"不可再分"措辞（§5，消除与 `split` 的表述冲突）；
  4. §14 补拆合验收 SQL ⑩~⑬；
  5. 明确"部分租赁的唯一载体是**单元层**"，资产层拆合语义收窄为**台账/权属边界**。
...
问题 2 **仅在数据结构层修复**：[ADR-0019](../adr/0019-asset-unit-and-occupancy-model.md) 的 V39–V41 已交付"资产 → 单元 → 占用"扩展层，但**写入路径尚未切换**——合同、招租、组合租赁、处置、退租仍写资产级 `lease_control_status`（全库 `asset_occupancy` 写入方只有 `occupation`/`selfuse`/`migration` 三处），`contract.asset_unit_id` 在 Java 侧从未被写入。因此"部分租赁"目前**仍无法在合同侧表达**；叠加 [ADR-0021](../adr/0021-asset-split-merge-rules.md) 记录的资产层拆合绕过单元（D-01/D-02），该问题的对外表现与 V1.0 诊断一致，只是根因从"模型缺失"变为"**链路未接入**"。问题 1、3 由本文件与 [ADR-0020](../adr/0020-bounded-contexts-and-aggregate-boundaries.md) 定义目标态。
...
| **计租单元** | `AssetUnit` | **合同/占用的最小挂载粒度**；任一时刻不可被两个主体同时排他占用。注意：**拆分不是"同一单元容纳多个占用"，而是"原子单元被替换为更细粒度的单元集合，原单元失效、不可再被引用"**（ADR-0021 决策 A） | "子资产"、"分间"；以及把 `split` 理解为"单元可再分" |
...
| **INV-13** | 拆分面积守恒：Σ 子标的面积 == 原标的面（\|Δ\| ≤ 0.01） | — | 服务层校验（ADR-0021 规则 S6/A4） | 服务层断言（越界即拒） + 结构日志面积核对 |
| **INV-14** | 拆合后「每个资产 ≥ 1 个有效单元」仍成立（INV-2 的运维延伸） | ★ | 服务层 + `v_asset_without_unit` | 验收 SQL ⑩ / 视图 |
| **INV-15** | 参与拆合的标的**无未收口占用（含 `reserving`）且无 `active` 招租** | ★ | 服务层校验（ADR-0021 规则 S3/S4/M2） | 验收 SQL ⑪ |
| **INV-16** | 价值/底价守恒：Σ 子 `originalValue` == 原 `originalValue`；Σ 子 `baseRent` == 原 `baseRent` | — | 服务层按面积分摊（ADR-0021 规则 S10/A4） | 验收 SQL ⑫ |
| **INV-17** | `structure_status ∈ {frozen, merged_out}` 的资产**不得出现在任何可租选择器** | — | `pickLeasable` / `resolveForLease` 加门禁（ADR-0021 规则 A8） | 验收 SQL ⑬ |
| **INV-18** | 拆合后编码确定性：`unit_no` / `asset_no` 可校验、可排序、可从编码反推归属 | — | `UnitNoGenerator`；禁用随机后缀 | 编码格式断言 |
| **INV-19** | **占用写入与收口对称闭合**：每条 `asset_occupancy` 行在其来源单据进入终态（驳回/撤回/退租完成/终止/作废/续签替换）时必须收口（`date_to` 非空） | ★ | 服务层成对交付（ADR-0021 决策 I） | 验收 SQL ⑮⑯ + 日检作业 |

---

### 7.4 拆分与合并（FR-MDM-001~004）

规则全文、26 项缺陷（D-01~D-26，其中 D-22~D-26 为**条件性**——接入占用后才触发）与备选方案见 **[ADR-0021 资产与计租单元拆分合并规则](../adr/0021-asset-split-merge-rules.md)**；落地批次与验收见 **[资产拆合落地实施方案](./资产拆合落地实施方案.md)**；本节只给出边界结论与判据。

#### 7.4.1 三层语义（先判层，再谈规则）

系统里"拆分/合并"有三个不同的东西，**不可混用**：

| 层 | 载体 | 何时用 | 是否承载"部分租赁" |
|----|------|--------|------------------|
| **① 单元层** | `asset_unit` | 把资产的**部分**出租 / 自用 / 占用 | ✅ **唯一载体**（ADR-0019 决策 A1） |
| **② 资产层** | `asset.parent_asset_id` / `structure_status` | 权证分证、土地分割、楼栋分立 | ❌ 只做**台账/权属边界** |
| **③ 组合租赁** | `lease_bundle` / `lease_bundle_item` | 一租户整包多单元 | ❌ 不产生新标的，是计费视图 |

**核心结论**：业务侧"资产支持部分租赁 → 拆分合并"应落在 **①**；把部分租赁表达为 ② 的资产拆分是**层错配**，会导致子资产无单元、面积/价值重复计数、数据权限失效（ADR-0021 D-01/D-06/D-07）。

#### 7.4.2 单元拆分规则（要点）

| # | 规则 |
|---|------|
| S1 | 门禁：资产"支持部分租赁"（**决策待定**：显式开关 `partial_lease_status` vs 由 `unit_count > 1` 派生；ADR-0021 推荐后者） |
| S2 | 拆分粒度是**单元**，不是资产 |
| S3/S4 | 前置：该单元无未收口占用（含 `reserving`）、无 `active` 招租发布 |
| S5 | 前置：无抵押 / 无处置中——**权属负担与拆分互斥**（§5 关键区分③） |
| S6 | 面积守恒：Σ 子单元面积 == 原单元面积；`area = 0` 占位单元一律拒绝拆分 |
| S7 | **只拆空置部分，在租部分原地不动**——避免合同脱标，这是"部分租赁"的正解 |
| S8 | 子单元状态仅由 `LeaseStatusDeriver` 派生，业务代码禁写 |
| S9 | 原单元**软删**保留历史身份；子单元编码 `UnitNoGenerator.childOf`（INV-18） |
| S10 | 底价/可租面积**按面积分摊**，不得整份复制（INV-16） |
| S11 | 原子事务 + 幂等键 `('unit_split', srcUnitId, hash(childAreas))` |
| S12 | **`merge` 是 `split` 的逆操作，必须同批交付**；可回滚窗口 = 下游（合同/招租/占用）未引用子单元之前 |
...
| M3 | 无抵押；权证须一并迁移或阻断 |
| M4 | 必须同资产、同项目、同分区、同 `asset_type` / `usage_type` |
| M5/M6 | 面积与底价相加；源单元软删 + 编码映射；结果标的 ≥ 1 个有效单元（INV-14） |
| M7 | 编码走 `UnitNoGenerator`，禁止时间戳/随机后缀 |

#### 7.4.4 资产层拆合（语义收窄）

适用场景白名单：权证分证、土地分割、楼栋分立。规则 A1–A11 见 ADR-0021 决策 D，其中五项必须在改造时同步落地：

1. **A3 子资产字段完整继承**（含 `zoneId`、`leaseArea`、`assetCompanyId`、`lifecycleStatus`、`ownershipStatus`）：`zoneId` 为空会导致分区汇总丢数与分区/楼层筛选查不到；`leaseArea` 为空会让可租面积口径从 `lease_area` 回退到 `area`，**直接改变 INV-1 的判定基准**；`assetCompanyId` 为空会影响名称回显、调拨/抵押按公司筛资产、以及抵押记录归属（**注意：不影响 RBAC**，数据范围读的是 `operating_company_id`）；
2. **A4 原值按面积分摊**，否则资产价值被放大 N 倍；
3. **A8 `frozen` / `merged_out` 不得出现在任何可租选择器**（INV-17），否则已合并退出的资产仍可被签约；
4. **A10 切分线必须与单元边界对齐**，单元不得跨子资产——跨界单元若在租则不可拆（S3），拆分将无解；
5. **A11 单元归属迁移与结构变更同事务**，不得异步补。

此外，拆合必须**先预演再提交**：dry-run 返回"将创建/失效的单元、面积与底价拆分结果、受影响的下游引用清单"，提交时校验预演快照未被并发改变。

**FR-MDM-004 落点**：父合同拆账单 / 原合同终止+子合同新签，**二选一，禁止无归属账单**；在**单元粒度**判定，不再依赖资产级 `attachedContracts` 计数。

#### 7.4.5 占用写入与收口必须对称闭合（INV-19）

拆合与"部分租赁"都建立在占用之上，因此有一条**跨 §7.4 与 §10.2 的硬规则**：**每新增一条占用写入路径，必须同批交付其对应收口路径**（ADR-0021 决策 I）。

原因是合同占用的 `dateTo` 传 `null`（文档注释：*"为空表示无固定期限（在租合同）"*），即**开区间**。若只写不收口：

1. 该占用行**永不结束**；
2. 派生器按占用集合重算 → 单元/资产**又派生回"在租"**；
3. 叠加 `EXCLUDE`（`WHERE exclusive`）→ **该单元永久锁死，再也租不出去**。

这**比现状更糟**（现状只是状态显示错，数据可修、单元还能租），所以它是"接入占用"的最大陷阱，而不是收尾工作。

**根因**：`v_unit_lease_status`（`V42`）**完全不引用 `contract` 表**——它只读 `asset_occupancy` / `lease_listing` / `asset.lifecycle_status`。因此"合同状态"与"派生租控状态"之间**唯一的耦合就是占用行**：

> **合同生命周期里每一个终止态，都必须显式收口占用。**

必须成对的**五处**：

| 占用写入点 | 同批必须交付的收口 |
|-----------|------------------|
| 提交审批预留（§10.3 决策 A） | 驳回 / 撤回 → `cancelReservation` |
| 合同生效 `occupy` | 退租完成 → `releaseBySubject` |
| 合同生效 `occupy` | 终止 / 作废 → `releaseBySubject` |
| 合同生效 `occupy` | **续签 / 转租时收口旧合同占用**（旧合同停留 `RENEWABLE`/`EXPIRING`，二者均属 `LeaseAreaGroups.OCCUPYING`，旧占用为开区间 → 新旧区间必然重叠 → `EXCLUDE` 拒绝 → **续签/转租无法生效**） |
| 合同生效 `occupy` | **合同到期（转 `EXPIRED`）→ 收口于 `endDate`**（`ScheduledJobs.contractExpiryScan` 目前只翻状态；不收口则开区间永远命中，**合同已到期而单元永久显示在租**） |

**自检问法**（建议进 CR checklist）：**"这条占用行，谁会把它关掉？"** 答不上来的写入点，就是一处泄漏。

> ⚠️ **检测手段当前未接线**：`LeaseStatusDeriver.refreshReconciled` 与 `v_asset_lease_status_reconcile` 已实现，但**全库无调用方**（`platform/job` 下也没有）。因此"漂移"既不为零、也不可见。改造的第一个动作应是**接只读比对 + 告警**（只统计不改数据），再动写入路径。
...
-- ⑫ 价值守恒（INV-16 / D-06）：冻结父资产的原值应等于其活动子资产合计
...
| 9 | **拆合不使子标的失去单元** | 验收 SQL ⑩ 为空（INV-14） |
| 10 | **拆合前置以占用表判定**（非派生列） | 验收 SQL ⑪ 仅剩 `attach_parent` 显式例外且已人工确认（INV-15） |
| 11 | **价值/底价守恒** | 验收 SQL ⑫ 为空（INV-16） |
| 12 | **冻结/已合并资产不可租** | 验收 SQL ⑬ 为空（INV-17） |
| 13 | **拆合可追溯且幂等** | `asset_structure_log` 有 `idempotency_key` 唯一索引；重复请求返回首次结果、无第二次副作用 |
| 14 | **占用写入与收口对称**（INV-19） | 验收 SQL ⑮⑯ 为空；退租 / 终止 / 作废 / 驳回 / 续签 后均无残留未收口占用 |
| 15 | **漂移可观测** | `refreshReconciled` 已接线为周期只读比对并告警；`v_asset_lease_status_reconcile` 的规模可见（当前方法已实现但**无调用方**） |
...
| V1.2 | 2026-09-14 | **依 [ADR-0021](../adr/0021-asset-split-merge-rules.md) 补齐拆分合并规则**：① 新增 §7.4（三层语义 + 单元拆合规则 S1–S11/M1–M7 + 资产层收窄 A1–A9），明确"部分租赁的唯一载体是**单元层**"；② 新增 INV-13~INV-18（§7.3）；③ 修正术语表 `AssetUnit` 的"不可再分"措辞（§5）；④ §14 补拆合验收 SQL ⑩~⑬ 与标准 9~13；⑤ 记录 26 项缺陷 D-01~D-26（含 4 项 P0：资产层拆合绕过单元/合并后影子可租/编码违规/合同链路未接入单元占用；以及 5 项**条件性 P0**：D-22~D-26 在"合同接占用"落地后会立刻触发，后果为单元永久锁死、续签/转租失效或**合同到期仍显示在租**——**比不改更糟**）；⑥ 新增 **INV-19「占用写入与收口对称闭合」**与 §7.4.5（决策 I），并记录 `refreshReconciled` **已实现但未接线**（检测手段存在 ≠ 在运行）；§2.1 现状诊断的"问题 2"表述同步收紧 |
