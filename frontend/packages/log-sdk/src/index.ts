...
export type AppType = 'admin-web' | 'h5-tenant' | 'h5-worker' | 'tenant-mp' | 'worker-mp';

export type LogSource = 'js' | 'promise' | 'api';

export interface LogSdkOptions {
  /** 上报地址。web 用相对路径 `/api/v1/public/app-logs`；小程序必须传绝对地址 */
  endpoint: string;
  appType: AppType;
  /** 自报用户 ID（仅排查用，后端不以它作鉴权依据） */
  getUserId?: () => number | null;
  /** 同指纹去重窗口，默认 5000ms */
  dedupeWindowMs?: number;
  /** true 时只 `console.error`，不发请求（本地开发用） */
  debug?: boolean;
  /**
   * 时间源。仅供测试注入，业务代码不要传。
   *
   * 之所以留这个口子而不是用假定时器：去重窗口的判断要有确定性断言，
   * 依赖运行时的时钟 mock 会让测试与 Node 版本耦合。
   */
  now?: () => number;
}
...
/** 上报体：与后端 `AppLogIngestRequest` 逐字段对应。 */
...
/**
 * 全局错误事件的形状（`window.onerror` / `unhandledrejection`）。
 *
 * <p>刻意声明成结构类型而不是 `any` 或 EventError：两个事件的字段并不一样
 * （`error` 事件给 message/filename/lineno，`rejection` 事件给 reason），
 * 全部声明为可选后，一处 handler 就能安全地处理两种事件，也省不掉字段级的类型检查。
 */
...
// ---------------------------------------------------------------------------
// 宿主环境探测（不引用 DOM 类型，全部经 typeof 守卫）
// ---------------------------------------------------------------------------

/**
 * 宿主对象容器。
 *
 * <p>`any` 是刻意的：本模块探测的是**未知宿主对象**（window / wx / navigator / Blob…），
 * 其形状由运行环境决定，而本文件刻意不引用 DOM 类型（见文件头）。声明成 `unknown`
 * 会让每一处访问都退化成断言，反而把真正的类型错误藏起来。
 */
...
export const isMiniProgram = (): boolean =>
  typeof host.wx !== 'undefined' && typeof host.wx.request === 'function';

/** 浏览器环境（有 window 与 fetch）。 */
const isBrowser = (): boolean =>
  typeof host.window !== 'undefined' && typeof host.fetch === 'function';
...
export const newTraceId = (): string => {
  ...
  return (
    Date.now().toString(16).padStart(12, '0') + Math.random().toString(16).slice(2, 22)
  ).slice(0, 32);
};
 ...
 */
export const clientFingerprint = (
  source: LogSource,
  appType: AppType,
  message: string,
  stack: string,
...
/** 从任意抛出物里取出堆栈（无则空串）。 */
export const stackOf = (value: unknown): string => {
...
export interface Logger {
  newTraceId(): string;
  /** 装全局钩子；web 环境有效，返回卸载函数。小程序请用 `App({ onError })`（见下）。 */
  install(): Uninstall;
  captureError(err: unknown, extra?: Record<string, unknown>): void;
  captureRejection(reason: unknown, extra?: Record<string, unknown>): void;
  captureApiFailure(info: ApiFailureInfo): void;
}
...
export const createLogger = (options: LogSdkOptions): Logger => {
  const {
    endpoint,
    appType,
    getUserId,
  ...
  const evictIfNeeded = (): void => {
    if (dedupe.size <= MAX_TRACKED_FINGERPRINTS) return;
  ...
  const currentUserAgent = (): string | undefined => {
    const ua = host.navigator?.userAgent;
    return typeof ua === 'string' ? truncate(ua, MAX_UA) : undefined;
  };

  const currentUrl = (): string | undefined => {
    const href = host.location?.href;
    return typeof href === 'string' ? truncate(href, MAX_URL) : undefined;
  };

  const send = (payload: LogPayload): void => {
    if (debug) {
  ...
  const beaconFallback = (body: string): void => {
    ...
    try {
      sendBeacon.call(
        host.navigator,
        endpoint,
        new host.Blob([body], { type: 'application/json' }),
      );
    } catch {
  ...
  const emit = (
    ...
    const fingerprint = clientFingerprint(source, appType, safeMessage, stack);
    const timestamp = now();
    ...
    const capped = capExtra(mergedExtra);
    if (capped) payload.extra = capped;
    const ua = currentUserAgent();
    if (ua) payload.ua = ua;
    const url = currentUrl();
    if (url) payload.url = url;
    const userId = getUserId?.();
    if (typeof userId === 'number') payload.userId = userId;
  ...
  return {
    ...
    install(): Uninstall {
      ...
      const onError = (event: ErrorEventLike): void => {
        ...
        const message = error
          ...
          : truncate(String(event?.message ?? 'unknown error'), MAX_MESSAGE);
        emit('js', message, stackOf(error), {
      ...
      const onRejection = (event: ErrorEventLike): void => {
        ...
        emit('promise', describeUnknown(reason), stackOf(reason), undefined);
      };
    ...
    captureError(err: unknown, extra?: Record<string, unknown>): void {
      emit('js', describeUnknown(err), stackOf(err), extra);
    },

    captureRejection(reason: unknown, extra?: Record<string, unknown>): void {
      emit('promise', describeUnknown(reason), stackOf(reason), extra);
    },
    ...
    captureApiFailure(info: ApiFailureInfo): void {
      ...
      const message = detail
        ? `${info.method} ${info.path} 失败${status}：${describeUnknown(detail)}`
